...
for dir in "\$PWD/.serena" "\$PWD/.beads" "\$PWD/.rustup" "\$PWD/.cargo" "\$HOME/go" "\$PWD/.opencode/cache"; do
  if [ ! -d "\$dir" ]; then
...
STARTED_SERVER=
# podman ps -q --filter name="\$CONTAINER_NAME" --filter status=running | grep -q .
if podman ps -q --filter "name=\${CONTAINER_NAME}" --filter status=running | grep -q . ; then
    echo "Container '\${CONTAINER_NAME}' is running."
    STARTED_SERVER=true
    # Add commands to execute if the container is running
    # For example: podman exec "\${CONTAINER_NAME}" ls /app
else
    echo "Container '\${CONTAINER_NAME}' is not running."
    STARTED_SERVER=false
    # Add commands to execute if the container is not running
    # For example: podman start "\${CONTAINER_NAME}"
fi
    ...
    if [ \${#NEW_ARGS[@]} -ge 3 ] && [ "\${NEW_ARGS[1]}" == "init" ] && [ "\${NEW_ARGS[2]}" == "--here" ]; then
      if [ -d ".specify" ]; then
        ...
        if ! command -v curl >/dev/null || ! command -v unzip >/dev/null; then
          ...
          ZIP_URL="https://github.com/github/spec-kit/releases/download/v0.0.85/spec-kit-template-opencode-sh-v0.0.85.zip"
          ZIP_FILE="spec-kit-template.zip"
          if curl -L "\$ZIP_URL" -o "\$ZIP_FILE"; then
            if unzip "\$ZIP_FILE"; then
              rm "\$ZIP_FILE"
      ...
      eval podman run --rm \$TTY_FLAG \
        ...
        \$SECRETS \$PATH_EXTENSION -e USER="\$USER" -e BUN_INSTALL_CACHE_DIR=/home/node/.cache/opencode/bun-cache \
...
if [ "\$START_SERVER" = true ]; then
  ...
  # Retry logic
  SERVER_CMD="podman run -d --replace --name \"\$CONTAINER_NAME\" --network \"\$NETWORK_NAME\" --publish \$PORT:\$PORT --userns=keep-id --security-opt=label=disable --cap-drop=ALL --cap-add=NET_BIND_SERVICE \$MOUNTS \$CONFIG_MOUNT \$CONFIG_ENV \$SECRETS -e USER=\"\$USER\" -e BUN_INSTALL_CACHE_DIR=/home/node/.cache/opencode/bun-cache \"\$IMAGE\" serve --hostname \"\$HOSTNAME\" --port \$PORT"
  ...
  if ! eval "\$SERVER_CMD" 2>&1 | tee /tmp/opencode_err.log; then
    ...
    NETWORK_NAME="host"
    SERVER_CMD="podman run -d --replace --name \"\$CONTAINER_NAME\" --net host --userns=keep-id --security-opt=label=disable --cap-drop=ALL --cap-add=NET_BIND_SERVICE \$MOUNTS \$CONFIG_MOUNT \$CONFIG_ENV \$SECRETS -e USER=\"\$USER\" -e BUN_INSTALL_CACHE_DIR=/home/node/.cache/opencode/bun-cache \"\$IMAGE\" serve --hostname \"\$HOSTNAME\" --port \$PORT"
    if [ "\$SERVER_MODE" = true ] && [ \${#NEW_ARGS[@]} -gt 0 ]; then
      SERVER_CMD="\$SERVER_CMD \"\${NEW_ARGS[@]}\""
    fi
...
# Command execution section (updated for serverless default TUI and explicit attach):
# Default: serverless TUI (--rm opencode on shared net).
# attach: server + client attach w/ URL.
if [ \${#NEW_ARGS[@]} -eq 1 ] && [[ "\${NEW_ARGS[0]}" == "--version" || "\${NEW_ARGS[0]}" == "--help" ]]; then
  # Lightweight: no net
  eval podman run --rm \$TTY_FLAG \
    --userns=keep-id --security-opt=label=disable --network=none \
    \$MOUNTS \$CONFIG_MOUNT \$CONFIG_ENV \
    \$SECRETS -e USER="\$USER" -e BUN_INSTALL_CACHE_DIR=/home/node/.cache/opencode/bun-cache \
    "\$IMAGE" "\${NEW_ARGS[@]}"
elif [ "\$SERVER_CLIENT_MODE" = true ]; then
  # Attach: Ensure server running, get IP, pass explicit URL + args
  if [ "\$NETWORK_NAME" == "host" ]; then SERVER_IP="127.0.0.1"
  elif [ "\$NETWORK_NAME" == "bridge" ]; then SERVER_IP="localhost"
  else SERVER_IP=\$(podman inspect "\$CONTAINER_NAME" --format "{{.NetworkSettings.Networks.\$NETWORK_NAME.IPAddress}}") || error_exit "Server IP fetch failed"; fi
  SERVER_URL="http://\${SERVER_IP}:\$PORT"
  eval podman run --rm \$TTY_FLAG \
    --network "\$NETWORK_NAME" \
    --userns=keep-id --security-opt=label=disable \
    \$MOUNTS \$CONFIG_MOUNT \$CONFIG_ENV \$SECRETS -e USER="\$USER" -e BUN_INSTALL_CACHE_DIR=/home/node/.cache/opencode/bun-cache \
    "\$IMAGE" attach "\$SERVER_URL" "\${NEW_ARGS[@]}"
  echo "Attached to server at \$SERVER_URL"
else
  # Default/other: Serverless TUI/multi-instance safe (shared net for consistency)
  CMD_ARGS=("\${NEW_ARGS[@]}")
  eval podman run --rm \$TTY_FLAG \
    --network "\$NETWORK_NAME" \
    --userns=keep-id --security-opt=label=disable \

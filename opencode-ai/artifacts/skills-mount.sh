...
log_info "Mounting (policy=$POLICY): $BRANCH_STR → $MOUNTPOINT"
mergerfs -o defaults,allow_other,policy="$POLICY",category.create=mfs "$BRANCH_STR" "$MOUNTPOINT" || log_error 'mergerfs mount failed'

...
for abs in "${BRANCHES[@]}"; do
  basename_branch="$(basename "$abs")"
  prefixed_mount="./skills/$basename_branch"
  mkdir -p "$prefixed_mount"
  if mountpoint -q "$prefixed_mount"; then
    $VERBOSE && log_info "Already mounted $prefixed_mount"
    continue
  fi
  $VERBOSE && log_info "Mount prefixed: $basename_branch ($abs → $prefixed_mount)"
  mergerfs -o allow_other,category.create="$POLICY" "$abs": "$prefixed_mount" || log_error "Mount $prefixed_mount failed"
done

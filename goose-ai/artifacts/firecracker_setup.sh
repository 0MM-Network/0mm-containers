...
LATEST_TAG=$(curl -s https://api.github.com/repos/firecracker-microvm/firecracker/releases/latest | grep '"tag_name":'
| sed -E 's/.*"([^"]+)".*/\1/')
...
DOWNLOAD_URL="https://github.com/firecracker-microvm/firecracker/releases/download/${LATEST_TAG}/firecracker-${LATEST_
TAG}-${ARCH}"
...
echo "Firecracker installed successfully. Run as non-root: $FIRECRACKER_BIN --api-sock
/tmp/firecracker/sockets/firecracker.sock"

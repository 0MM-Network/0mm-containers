...
if [ "$1" = 'server' ]; then
    ...
    set -- vault server \
        ...
        "$@"
elif vault "$1" -h 2>&1 | grep -q "vault $1"; then
...
if [ "$1" = 'vault' ]; then
    ...
    # Removed SKIP_SETCAP handling to enforce mlock; capabilities granted externally via Podman --cap-add=IPC_LOCK
...
# If running server mode and TRANSIT_ADDR is set, inject it into the config template
if [ "$1" = 'vault' ] && [ "$2" = 'server' ] && [ -n "$TRANSIT_ADDR" ]; then
    sed -i "s|ADDRESS_PLACEHOLDER|$TRANSIT_ADDR|g" /vault/config/server.hcl
fi
